...
public class MainWindow : Window, IDisposable
{
    ...
    private int lootDistributionSelectedTeamIndex = 0; // Selected team for loot distribution
    private string lootDistributionPlanOutput = ""; // Generated plan output
    private Helpers.LootDistributionCalculator.DistributionPlan? currentDistributionPlan = null;
    ...
    private void DrawCalculateLootDistributionTab()
    {
        ...
        var teamNames = plugin.Configuration.RaidTeams.Select(t => t.Name).ToArray();
        if (ImGui.Combo("##TeamSelector", ref lootDistributionSelectedTeamIndex, teamNames, teamNames.Length))
        {
            // Team changed
        }
        ...
        // Buttons
        var selectedTeam = plugin.Configuration.RaidTeams[lootDistributionSelectedTeamIndex];
        
        if (ImGui.Button("Generate Plan##GeneratePlan", new Vector2(200, 30)))
        {
            GenerateLootDistributionPlan(selectedTeam, useCurrentState: false);
        }

        ImGui.SameLine();

        if (ImGui.Button("Generate Plan from Present State##GeneratePlanPresent", new Vector2(250, 30)))
        {
            GenerateLootDistributionPlan(selectedTeam, useCurrentState: true);
        }
        ...
        // Copy button at top
        if (ImGui.Button("Copy Plan to Clipboard##CopyButton", new Vector2(200, 25)))
        {
            ImGui.SetClipboardText(lootDistributionPlanOutput);
        }

        ImGui.Spacing();
        
        // Display area - using InputTextMultiline for selectability
        if (ImGui.BeginChild("DistributionPlanOutput", new Vector2(-1, 1200), true))
        {
            ImGui.InputTextMultiline("##DistributionPlanText", ref lootDistributionPlanOutput, 100000, 
                new Vector2(-1, -1), ImGuiInputTextFlags.ReadOnly | ImGuiInputTextFlags.AllowTabInput);
            ImGui.EndChild();
        }
    }

    private void GenerateLootDistributionPlan(Models.RaidTeam team, bool useCurrentState)
    {
        try
        {
            currentDistributionPlan = Helpers.LootDistributionCalculator.CalculateDistribution(team, useCurrentState);
            lootDistributionPlanOutput = FormatDistributionPlanOutput(currentDistributionPlan);
        }
        catch (Exception ex)
        {
            lootDistributionPlanOutput = $"Error generating plan: {ex.Message}\n{ex.StackTrace}";
        }
    }

    private string FormatDistributionPlanOutput(Helpers.LootDistributionCalculator.DistributionPlan plan)
    {
        var sb = new System.Text.StringBuilder();
        
        sb.AppendLine("=== LOOT DISTRIBUTION PLAN ===");
        sb.AppendLine($"Total Weeks: {plan.TotalWeeks}");
        sb.AppendLine();

        // Include starting state
        foreach (var line in plan.StartingState)
        {
            sb.AppendLine(line);
        }
        sb.AppendLine();
        ...
        foreach (var week in plan.Weeks)
        {
            ...
            sb.AppendLine($"Floors to Run: {string.Join(", ", week.FloorsToRun)}");
            sb.AppendLine("Allocations:");
            foreach (var allocation in week.Allocations)
            {
